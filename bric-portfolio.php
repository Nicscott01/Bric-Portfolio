...
class BricPortfolio {
	...
	public function query_options( $query ) {
		...
		if ( !empty( $this->options['query_options'] ) && is_array( $this->options['query_options'] ) ) {
	...
	public function get_content( $the_content ) {
		...
		if ( !class_exists( 'PhotoGallery') ) {
			include_once( $this->includes . '/PhotoGallery.class.php' );
		}
...
$BricPortfolio = new BricPortfolio();